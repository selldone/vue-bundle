...
if (getQueryStringValue("shop_id")) {
  ...
  if (window.Selldone.shopId !== sessionStorage.getItem("shop_id"))
    needReset = true;
...
if (getQueryStringValue("api")) {
  ...
  if (window.Selldone.appApiKey !== sessionStorage.getItem("api"))
    needReset = true;
...
const accessToken: any = document.head.querySelector(
  'meta[name="app-access-token"]'
);
window.Selldone.accessToken = accessToken ? accessToken.content : null;
if (window.Selldone.accessToken) {
  console.log(
    "Access token set by meta tags. (Feed by your application backend)"
  );
}
