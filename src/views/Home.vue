    ...
    <div>
        ...
        <v-simple-table>
            <template v-slot:default>
                ...
                <tr>
                    <td>Shop ID</td>
                    <td class="max-width">{{ shopId }} <v-alert v-if="!shopId" class="mt-2" dense colored-border border="left" color="red" type="error">Set shop_id in your application url query!</v-alert> </td>
                    <td class="font-italic">Auto set by app url</td>
                </tr>
                <tr>
                    <td>App API</td>
                    <td class="max-width">{{ appApiKey }} <v-alert v-if="!appApiKey"  class="mt-2" dense colored-border border="left" color="red" type="error">Set api in your application url query!</v-alert> </td>
                    <td class="font-italic">Auto set by app url</td>
                </tr>
...
<script>
    export default {
        ...
        methods: {
            getSuggestionUsers() {
                window.Selldone.ShopApi.getSuggestionShopStaffs('paj')
                    .then(({data}) => {
